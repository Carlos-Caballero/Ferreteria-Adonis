...
class InventarioController {
  ...
  async index ({ request, response, view }) {
    ...
    const inventarios = await Inventario.query().with('productos').fetch();
  ...
  async store ({ request, response }) { 
    ...
    const inventario = await Inventario.create(request.all());
  ...
  async update ({ params, request, response }) {
    let inventarios = await Inventario.find(params.id)
    inventarios.merge(request.all())
